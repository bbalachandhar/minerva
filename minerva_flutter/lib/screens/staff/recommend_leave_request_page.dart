...
class RecommendLeaveRequestPage extends StatefulWidget {
  const RecommendLeaveRequestPage({Key? key}) : super(key: key);

  @override
  State<RecommendLeaveRequestPage> createState() =>
      _RecommendLeaveRequestPageState();
}

class _RecommendLeaveRequestPageState extends State<RecommendLeaveRequestPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Recommend Leave Requests'),
        elevation: 0,
      ),
      body: _isLoading
          ...
          : Consumer<LeaveRequestProvider>(
              builder: (context, leaveProvider, _) {
                ...
                final pendingRequests = leaveProvider.leaveRequests
                    .where(
                      (req) => req.recommenderStatus.toLowerCase() == 'pending',
                    )
                    .toList();
                  ...
                  return Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 16),
                        const Text(
                          'No pending leave requests to recommend',
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ElevatedButton.icon(
                  onPressed: () {
                    _showRecommendDialog(request, false);
                  },
                ...
                ElevatedButton.icon(
                  onPressed: () {
                    _showRecommendDialog(request, true);
                  },
  ...
  void _showRecommendDialog(StaffLeaveRequest request, bool isApprove) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(
          isApprove ? 'Recommend Leave Request' : 'Reject Leave Request',
        ),
        content: Column(
          ...
          children: [
            ...
            Text(
              isApprove
                  ? 'Do you want to recommend this leave request?'
            ...
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                    isApprove
                        ? 'Leave request recommended successfully'
                        : 'Leave request rejected successfully',
                  ),
                ),
              );
              // TODO: Call API to update recommender_status
            },
            child: Text(
              isApprove ? 'Recommend' : 'Reject',
