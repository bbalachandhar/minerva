...
class AppConfigProvider extends ChangeNotifier {
  ...
  String _baseUrl = '';
  String _cachedLogo = '';
  String _cachedSchoolName = 'minerva';
  String _cachedSchoolCode = '';
  String _cachedPrimaryColor = '#7C3AED';
  String _cachedSecondaryColor = '#FFFFFF';
  ...
  bool get studentProfileEdit =>
      (_appConfig?['student_profile_edit']?.toString() ?? '0') == '1';
  bool get staffProfileEdit =>
      (_appConfig?['staff_profile_edit']?.toString() ?? '0') == '1';
  ...
  /// Returns the institution type stored in settings (e.g. 'School', 'University').
  String get institutionType {
    final fromConfig = _appConfig?['institution_type']?.toString() ?? '';
    if (fromConfig.isNotEmpty) return fromConfig;
    return _cachedInstitutionType;
  }

  /// Returns true only when the institution is configured as a school.
  bool get isSchoolType => institutionType.toLowerCase() == 'school';
  ...
  Future<void> loadAppConfig() async {
    ...
    try {
      ...
      if (_cachedSchoolName.isEmpty) _cachedSchoolName = 'minerva';

      _cachedPrimaryColor = await AppConfig.getPrimaryColor();
      if (_cachedPrimaryColor.isEmpty) _cachedPrimaryColor = '#7C3AED';

      _cachedSecondaryColor = await AppConfig.getSecondaryColor();
      if (_cachedSecondaryColor.isEmpty) _cachedSecondaryColor = '#FFFFFF';

      _cachedAttendanceType = await AppConfig.getAttendanceType();
      _cachedInstitutionType = await AppConfig.getInstitutionType();
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '[AppConfigProvider] API status: $status, isSuccess: $isSuccess',
        );
        debugPrint('[AppConfigProvider] --- FULL API RESPONSE ---');
        config.forEach((k, v) {
          if (v is! List) debugPrint('  $k: $v');
        });
        debugPrint('[AppConfigProvider] --- END RESPONSE ---');
      }

      if (config['isNetworkError'] == true) {
      ...
      if (isSuccess && config.length > 2) {
        ...
        if (kDebugMode) {
          debugPrint('[AppConfigProvider] getAppLogo() after save: $newLogo');
          debugPrint('[AppConfigProvider] appLogo getter: $appLogo');
        }
      ...
      _error = e.toString();
      _isNetworkError = true;
      if (kDebugMode) debugPrint('[AppConfigProvider] loadAppConfig error: $e');
    }
  ...
  /// Fast refresh of primary/secondary colors from SharedPrefs (no network call).
  /// Call this immediately after saving app configuration to shared prefs so the
  /// MaterialApp theme rebuilds without waiting for a full network fetch.
