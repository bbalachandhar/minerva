...
class User_model extends CI_Model
{
    ...
    {
        $this->db->select('users.*,students.firstname,students.image,students.middlename,students.lastname,students.guardian_name,classes.id AS `class_id`,classes.class,sections.id AS `section_id`,sections.section,students.admission_no,students.mobileno,students.email,student_session.id as student_session_id');
        $this->db->from('users');
    ...
    {
        $sql   = "SELECT users.* FROM `users` join students on students.parent_id = users.id WHERE users.id = " . $student_id;
        $query = $this->db->query($sql);
        return $query->row_array();
    }
    ...
    public function getStudentCurrency($student_id)
    {       
        
        $this->db->select('users.*,currencies.name,currencies.id,currencies.symbol,currencies.base_price');
        $this->db->join('currencies', 'currencies.id = users.currency_id');
        $this->db->from('users');
        $this->db->where('user_id', $student_id);
        $query = $this->db->get();
        return $query->result();
    ...
    public function getstudentcurrentcurrency($student_id)
    {
        $this->db->select('users.*,currencies.*');
        $this->db->join('currencies', 'currencies.id = users.currency_id');
        $this->db->from('users');
        $this->db->where('user_id', $student_id);
        $query = $this->db->get();
