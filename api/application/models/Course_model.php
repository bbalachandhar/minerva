...
class Course_model extends CI_Model
{
    ...
    public function coursedetail($courseid)
    {
        ...
        $result['assignment_count']   = count($this->totalassignmentbycourse($courseid));
        $result['exam_count']   = count($this->totalexambycourse($courseid));	
        return $result;
    }
	...
	public function totallessonbycourse($courseID)
    {
        ...
        $this->db->join('online_course_section', 'online_course_section.id = online_course_lesson.course_section_id');
    ...
    public function totalassignmentbycourse($courseID) {
        $this->db->select('(online_course_assignment.id) as total_assignment')->from('online_course_assignment');
    ...
    public function totalexambycourse($courseID) {
        $this->db->select('(online_course_exam.id) as total_exam')->from('online_course_exam');
    ...
    public function getsectionbycourse($course_id, $student_id)
    {
        ...
        foreach ($result as $key => $sectionList_value) {
            ...
            $chackassignmentcount   = count($this->numberofassignment($sectionList_value->id));
            $chackexamcount         = count($this->numberofexam($sectionList_value->id));
            $chackcount             = $chackquizcount + $chacklessoncount+$chackassignmentcount+$chackexamcount;
            if ($chackcount > 0) {              
    ...
    public function numberofassignment($sectionid)
    {
        $this->db->select('online_course_assignment.*')->from('online_course_assignment');
        $this->db->where('online_course_assignment.course_section_id', $sectionid);
        $query = $this->db->get();
        return $query->result_array();
    }
   public function numberofexam($sectionid)
    ...
    public function lessonquizbysection($sectionid, $student_id, $course_id) 
    {
        ...
        foreach ($result as $key => $result_value) {
            ...
            } elseif ($result_value->type == 'assignment') {
                $ptype       = 3;
                $result[$key]->quiz_status = 0;
                
            } elseif ($result_value->type == 'exam') {
                $ptype       = 4;
                $result[$key]->quiz_status = 0;
    ...
    public function getexam($id) {
        $this->db->select('online_course_exam.id,online_course_exam.course_id,online_course_exam.course_section_id,online_course_exam.exam, online_course_exam.attempt,
        online_course_exam.exam_from,IFNULL(online_course_exam.exam_to,"") as exam_to,online_course_exam.is_quiz,online_course_exam.auto_publish_date,
        online_course_exam.time_from,online_course_exam.time_to,online_course_exam.duration,online_course_exam.passing_percentage,online_course_exam.description,
        online_course_exam.publish_result,online_course_exam.answer_word_count,online_course_exam.is_active,online_course_exam.is_marks_display,
        online_course_exam.is_neg_marking,online_course_exam.is_random_question,online_course_exam.is_rank_generated,online_course_exam.publish_exam_notification,
        online_course_exam.publish_result_notification,online_course_exam.created_at,online_course_exam.updated_at');
	...
	public function online_course_exam_result($student_id,$exam_id){        
        ...
        $this->db->join('online_course_exam_result',"online_course_exam_result.question_id=online_course_exam_marks.id and online_course_exam_result.student_id=$student_id","left");
    ...
    //code added//
    public function get_exam($id = null, $publish = null){
        $this->db->select('online_course_exam.*')->from('online_course_exam');
        if ($id != null) {
            $this->db->where('online_course_exam.id', $id);
        } else {
            $this->db->order_by('online_course_exam.id');
        }
        if ($publish != null) {
    ...
    public function getsubmitstatus($student_id,$exam_id, $user_type) {
        $this->db->join('online_course_exam_marks','online_course_exam_marks.id=online_course_exam_result.question_id','left');
        if( $user_type=='student'){
            $this->db->where('online_course_exam_result.student_id',$student_id);
        }else if($user_type=='guest'){
            $this->db->where('online_course_exam_result.guest_id',$student_id);  
        }
        $this->db->where('online_course_exam_marks.online_course_exam_id', $exam_id);
    ...
    public function getExamQuestions($exam_id = null, $random_type = false){
        $this->db->select('IFNULL(online_course_exam_marks.online_course_exam_id,0) as `online_course_exam_id`,online_course_exam_marks.question_id,online_course_exam_question.*,online_course_tag.tag_name,
     ...
     public function getStudentAttemts($student_id,$exam, $user_type){
        if( $user_type=='student'){
            $this->db->where('student_id',$student_id);
        }else if( $user_type=='guest'){
    ...
    {
        $query = "SELECT count(online_course_exam_marks.id) as total_question,SUM(CASE WHEN online_course_exam_question.question_type= 'descriptive' THEN 1 ELSE 0 END) AS total_descriptive FROM `online_course_exam_marks` left join online_course_exam_question on online_course_exam_question.id=online_course_exam_marks.question_id where online_course_exam_marks.online_course_exam_id =" . $this->db->escape($exam_id) . " group by online_course_exam_marks.online_course_exam_id";
        $query = $this->db->query($query);
        return $query->row();
    } 

    public function student_exam_result($exam_id)
    {
        $query = "SELECT count(online_course_exam_marks.id) as total_question,SUM(CASE WHEN online_course_exam_question.question_type= 'descriptive' THEN 1 ELSE 0 END) AS total_descriptive FROM `online_course_exam_marks` left join online_course_exam_question on online_course_exam_question.id=online_course_exam_marks.question_id where online_course_exam_marks.online_course_exam_id =" . $this->db->escape($exam_id) . " group by online_course_exam_marks.online_course_exam_id";
        $query = $this->db->query($query);
        return $query->row();
    ...
    public function savecourseexam($data, $studentid,$guestid,$usertype,$exam_id)
    {
        $status = 0;
        ...
        $this->db->join('online_course_exam_marks', 'online_course_exam_marks.id = online_course_exam_result.question_id','left');
        if($usertype=="student"){
            $this->db->where('online_course_exam_result.student_id', $studentid);
        }else{
            $this->db->where('online_course_exam_result.guest_id', $guestid);
        }
        $this->db->where('online_course_exam_marks.online_course_exam_id', $exam_id);
        $q = $this->db->get('online_course_exam_result');
        if ($q->num_rows() > 0) {
            $status = 2;
        } else {
            $this->db->insert_batch('online_course_exam_result', $data);
            $status = 1;
        }
        //======================Code End==============================
