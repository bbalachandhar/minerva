       ...
       public function pay() {
        ...
        $data['student_data'] = $this->student_model->get($data['params']['student_id']);
           ...
           $payment_data['pay_to_email'] =$pay_method->api_email;
            ...
            $payment_data['amount'] =convertBaseAmountCurrencyFormat($session_params['payment_detail']->fine_amount+$session_params['total']  - $session_params['applied_fee_discount']+ $session_params['gateway_processing_charge']);
            ...
            $data['form_fields']=$payment_data;
             $session_params['transaction_id']=$payment_data['transaction_id'];
              $payment_data['fee_discount_group']=$session_params['fee_discount_group'];
            $ins_data=array(
           ...
           $json_array = array(
                    'amount' => convertBaseAmountCurrencyFormat($session_params['payment_detail']->fine_amount+$session_params['total'] - $session_params['applied_fee_discount']+ $session_params['gateway_processing_charge']),
                    'date' => date('Y-m-d'),
                    'amount_discount' => 0,
