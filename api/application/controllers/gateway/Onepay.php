    ...
    public function pay() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $payment_data = array(
                ...
                'vpc_ReturnURL' => base_url() . 'gateway/onepay/complete/' . $session_id,
                'vpc_SHIP_City' => '',
                'vpc_SHIP_Country' => '',
                'vpc_SHIP_Provice' => '',
                'vpc_SHIP_Street01' => '',
...
$errorExists = false;
if (strlen($SECURE_SECRET) > 0 && isset($_GET["vpc_TxnResponseCode"]) && $_GET["vpc_TxnResponseCode"] != "7" && $_GET["vpc_TxnResponseCode"] != "No Value Returned") {
    ksort($_GET);
    $md5HashData = "";
...
if($hashValidated=="CORRECT" && $txnResponseCode=="0"){
    $transStatus = "success";
            ...
            $payment_id = isset($_GET["vpc_MerchTxnRef"]) ? $_GET["vpc_MerchTxnRef"] : '';
            if ($payment_id) {
                // Get session_id from URL parameter
                $session_id = $this->uri->segment(3);
