...
def solve(data: dict) -> dict:
    ...
    for i, load in enumerate(loads):
        ppw = load["periods_per_week"]
        if load.get("distribute_evenly", False):
            upper = (ppw + D - 1) // D          # ceil(ppw / D)
            lower = ppw // D if ppw >= D else 0  # floor(ppw / D)
            for d in range(D):
                day_sum = sum(_x(i, d, p) for p in range(P))
                model.add(day_sum <= upper)
                if lower > 0:
                    model.add(day_sum >= lower)

        # min_per_day: at least 1 period every working day
        if load.get("min_per_day", False) and ppw >= D:
            for d in range(D):
                day_sum = sum(_x(i, d, p) for p in range(P))
                model.add(day_sum >= 1)
