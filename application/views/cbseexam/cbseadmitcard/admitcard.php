                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
						...
						<div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('cbse_exam_admit_card', 'can_view')) { ?>
                            <a type="button" class="btn btn-sm btn-primary" href="<?php echo site_url('cbseexam/cbseadmitcard/index') ?>"  data-originaltitle="<?php echo $this->lang->line('design_admit_card')?>" ><?php echo $this->lang->line('design_admit_card')?></a> 
                            <?php } ?>              
                        </div>
                    </div>
					
					<?php if ($this->rbac->hasPrivilege('cbse_exam_print_admit_card', 'can_view')) { ?>
                    <div class="box-body">
                        <form role="form" action="<?php echo site_url('cbseexam/cbseadmitcard/admitcard') ?>" method="post" class="row">
                            ...
                            <div class="col-sm-6 col-lg-3 col-md-6 col20">
                                <div class="form-group">
                                    ...
                                    <option value=""><?php echo $this->lang->line('select'); ?></option>
                                    <?php
                                    foreach ($getexamlist as $exam) { ?>
                                    <option value="<?php echo $exam->id ?>" <?php
                                    if (set_value('exam_id') == $exam->id) {
                                        echo "selected=selected";
                                    } ?>>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <button type="submit" name="search" value="search_filter" class="btn btn-primary pull-right btn-sm checkbox-toggle mb10"><i class="fa fa-search"></i> <?php echo $this->lang->line('search'); ?></button>
                                </div>
                            </div>
                        </form>
...
if (isset($get_cbse_exam_students_list) && (!empty($get_active_admitcard))) {
   
    ?>
                       
                        <form method="post" action="<?php echo base_url('cbseexam/cbseadmitcard/printcard') ?>" id="printCard">
        ...
        foreach ($get_cbse_exam_students_list as $student_key => $student_value) {
            ?>
                                                        <tr>
                                                            ...
                                                            <td><?php if($student_value->gender){ echo $this->lang->line(strtolower($student_value->gender)); } ?></td>
...
<script type="text/javascript">
    ...
    function getSectionByClass(class_id, section_id) {
	 ...
	 $(document).on('change', '#section_id', function (e) {
        ...
        getexamBySectionClass(class_id, section_id);
    });
function getexamBySectionClass(class_id, section_id) {
        ...
        if (class_id != "") {
            ...
            $.ajax({
                type: "GET",
                url: base_url + "cbseexam/exam/getByClasssection",
