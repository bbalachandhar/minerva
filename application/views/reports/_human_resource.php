...
<div class="report-nav">
    <div class="report-nav-header">
        ...
        <h3><?php echo $this->lang->line('human_resource_report'); ?></h3>
    </div>
    <div class="report-tiles">
        <?php if ($this->rbac->hasPrivilege('staff_report', 'can_view')) { ?>
        <a href="<?php echo base_url(); ?>report/staff_report" class="report-tile <?php echo set_SubSubmenu('Reports/human_resource/staff_report'); ?>">
            <span class="tile-icon"><i class="fa fa-id-badge"></i></span> <?php echo $this->lang->line('staff_report'); ?>
        </a>
        <?php } if ($this->rbac->hasPrivilege('staff_profile_completeness_report', 'can_view')) { ?>
        <a href="<?php echo base_url(); ?>report/staffprofilecompleteness" class="report-tile <?php echo set_SubSubmenu('Reports/human_resource/staffprofilecompleteness'); ?>">
            <span class="tile-icon"><i class="fa fa-check-circle"></i></span> Staff Profile Completeness Report
        </a>
        <?php } if ($this->rbac->hasPrivilege('payroll_report', 'can_view')) { ?>
        <a href="<?php echo base_url(); ?>admin/payroll/payrollreport" class="report-tile <?php echo set_SubSubmenu('Reports/attendance/attendance_report'); ?>">
            <span class="tile-icon"><i class="fa fa-money"></i></span> Paid Payroll Report
        </a>
        <a href="<?php echo base_url(); ?>admin/payroll/payrollreport_generated" class="report-tile">
            <span class="tile-icon"><i class="fa fa-file-text"></i></span> Generated Payroll Report
        </a>
        <?php } ?>
        <a href="<?php echo base_url(); ?>report/staff_birthday_list" class="report-tile <?php echo set_SubSubmenu('Reports/human_resource/staff_birthday_list'); ?>">
            <span class="tile-icon"><i class="fa fa-birthday-cake"></i></span> Staff Birthday List
        </a>
    </div>
</div>
