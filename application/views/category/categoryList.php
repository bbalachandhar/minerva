    ...
    <section class="content">
        <div class="crud-page">
            <div class="crud-header">
                <h2>
                    <span class="header-icon"><i class="fa fa-tags"></i></span>
                    <?php echo $this->lang->line('category_list'); ?>
                    <span class="badge-count"><?php echo count($categorylist); ?></span>
                </h2>
                <?php if ($this->rbac->hasPrivilege('student_categories', 'can_add')): ?>
                <button class="btn-add-new" onclick="openAddModal()"><i class="fa fa-plus"></i> <?php echo $this->lang->line('create_category'); ?></button>
                <?php endif; ?>
            </div>
            ...
            <div class="crud-panel">
                ...
                <div class="empty-state"><i class="fa fa-folder-open-o"></i><p><?php echo $this->lang->line('no_record_found'); ?></p></div>
                <?php else: ?>
                <table class="crud-table">
                    <thead><tr><th style="width:50px">#</th><th><?php echo $this->lang->line('category'); ?></th><th><?php echo $this->lang->line('category_id'); ?></th><th style="width:100px;text-align:center"><?php echo $this->lang->line('action'); ?></th></tr></thead>
                    <tbody>
                        <?php $count = 1; foreach ($categorylist as $category): ?>
                        <tr>
                            <td style="color:#adb5bd;font-size:12px;"><?php echo $count++; ?></td>
                            <td class="item-name"><?php echo $category['category']; ?></td>
                            <td><span class="item-id">#<?php echo $category['id']; ?></span></td>
                            <td style="text-align:center">
                                <?php if ($this->rbac->hasPrivilege('student_categories', 'can_edit')): ?>
                                <a href="javascript:void(0)" class="btn-action btn-action-edit" data-id="<?php echo $category['id']; ?>" data-name="<?php echo htmlspecialchars($category['category'], ENT_QUOTES); ?>" onclick="openEditModal(this)" data-toggle="tooltip" title="<?php echo $this->lang->line('edit'); ?>"><i class="fa fa-pencil"></i></a>
                                <?php endif; ?>
                                <?php if ($this->rbac->hasPrivilege('student_categories', 'can_delete')): ?>
                                <a href="<?php echo base_url(); ?>category/delete/<?php echo $category['id']; ?>" class="btn-action btn-action-delete" data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="return confirm('<?php echo $this->lang->line('delete_confirm'); ?>');"><i class="fa fa-trash"></i></a>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
...
<div class="crud-modal-overlay" id="crudModal">
    <div class="crud-modal">
        <div class="crud-modal-header">
            <h3 id="modalTitle"><i class="fa fa-plus-circle" style="color:#5b73e8;margin-right:6px;"></i> <?php echo $this->lang->line('create_category'); ?></h3>
            <button class="crud-modal-close" onclick="closeModal()">&times;</button>
        </div>
        <form id="crudForm" action="<?php echo site_url('category/create'); ?>" method="post">
            <?php echo $this->customlib->getCSRF(); ?>
            <div class="crud-modal-body">
                <label><?php echo $this->lang->line('category'); ?> <span class="req">*</span></label>
                <input type="text" name="category" id="modalInput" class="crud-input" placeholder="<?php echo $this->lang->line('category'); ?>" value="<?php echo set_value('category'); ?>">
                <span class="text-danger" style="font-size:12px;"><?php echo form_error('category'); ?></span>
            </div>
...
.crud-modal { background:#fff; border-radius:12px; width:420px; max-width:92vw; box-shadow:0 20px 60px rgba(0,0,0,.15); position:fixed; top:50%; left:50%; transform:translate(-50%,-50%) scale(.95); opacity:0; transition:transform .25s ease, opacity .25s ease; z-index:100000; }
...
function openAddModal() {
    $('#modalTitle').html('<i class="fa fa-plus-circle" style="color:#5b73e8;margin-right:6px;"></i> <?php echo $this->lang->line('create_category'); ?>');
    $('#crudForm').attr('action', '<?php echo site_url('category/create'); ?>');
    $('#modalInput').val('');
...
function openEditModal(el) {
    ...
    $('#modalTitle').html('<i class="fa fa-pencil" style="color:#5b73e8;margin-right:6px;"></i> <?php echo $this->lang->line('edit_category'); ?>');
    $('#crudForm').attr('action', '<?php echo base_url(); ?>category/edit/' + id);
    $('#modalInput').val(name);
    $('#crudModal').addClass('show');
    setTimeout(function(){ $('#modalInput').focus(); }, 200);
}
...
<?php if (form_error('category')): ?>$(function(){ openAddModal(); });<?php endif; ?>
</script>
