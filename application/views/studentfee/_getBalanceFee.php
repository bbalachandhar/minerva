<div class="form-horizontal balanceformpopup">
    ...
    <div class="box-body">
        ...
        $fee_base_amount = isset($student_fees) && $student_fees !== '' ? $student_fees : $balance;
        ?>
        ...
        <input type="hidden" id="fee_balance" value="<?php echo str_replace(',', '', $balance); ?>">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="text" autofocus="" class="form-control modal_amount" id="amount" value="<?php echo $fee_base_amount; ?>">
            ...
            <div class="col-sm-9 col-lg-9 col-md-9">
                <?php if (!empty($discount_not_applied)) { ?>
                    <div class="checkbox-fees-scroll">
                        ...
                        <div class="row">
                            <?php foreach ($discount_not_applied as $index => $discount_value) { ?>
                                <div class="col-md-12">
                                    <div class="row">
                                        <div class="col-md-7 col-sm-7 col-xs-7">
                                            <label class="checkbox-inline pt0">
                                                ...
                                                ?>" data-type="<?php echo $discount_value->type; ?>" data-percentage="<?php echo ($discount_value->type == "percentage") ? ($discount_value->percentage) : "0"; ?>">
                                                <?php echo $discount_value->name; ?><?php if ($discount_value->code) { echo " (" . $discount_value->code . ")"; } ?><?php if ($discount_value->type == "fix" && isset($discount_value->custom_amount) && $discount_value->custom_amount > 0) { echo " - " . $currency_symbol . $discount_value->custom_amount; } ?>
                                            </label>
                                        </div>
                                        <div class="col-md-3 col-sm-3 col-xs-3 text text-center"><?php echo $discount_value->remaining_discount_limit; ?></div>
                                        <div class="col-md-2 col-sm-2 col-xs-2 text text-right">
                                            <?php
                                            $displayed_value = "";
                                            if ($discount_value->type == "fix") {
                                                if (isset($discount_value->custom_amount) && $discount_value->custom_amount > 0) {
                                ...
                                <?php if (($index + 1) % 1 === 0 && $index + 1 !== count($discount_not_applied)) { ?>
                                    </div>
                                    <div class="row">
                                <?php } ?>
                            <?php } ?>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9 col-lg-9 col-md-9">
                ...
                <label class="radio-inline hidden">
                    <input type="radio" name="payment_mode_fee" value="Advance">
                </label>
...
$(document).ready(function () {
    ...
    var fee_balance_val = <?php echo str_replace(',', '', $fee_base_amount); ?>;
    var original_amount_input_val = parseFloat($('#amount').val()) || 0;
    ...
    function applyAdvanceLogic() {
        ...
        $('#amount_discount_from_advance').val(discount_to_apply.toFixed(2));
    ...
    $('input[name="use_paid_advance"]').on('change', function () {
        applyAdvanceLogic();
        setNoteFromSelections();
    });
    $('input[name="use_discount_advance"]').on('change', function () {
        applyAdvanceLogic();
        setNoteFromSelections();
    });
    $(document).on('change', 'input[name="fee_discount_group[]"]', function () {
        setNoteFromSelections();
    });
