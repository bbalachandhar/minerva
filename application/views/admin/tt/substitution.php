    ...
    <div class="row" style="align-items:flex-end;">
      <div class="col-md-4">
        <label style="font-size:12px;font-weight:600;color:#555;margin-bottom:4px;">
          <i class="fa fa-user-times text-danger"></i> Absent Teacher(s) <span class="text-danger">*</span>
        </label>
        <select class="form-control" id="absent_staff" multiple="multiple">
...
$(function(){
  ...
  $('#absent_staff').select2({ placeholder: '-- Select Absent Teachers --', allowClear: true, width: '100%' });

  // Staff name lookup
  var staffNames = {};
  <?php foreach ($staff_list as $st): ?>
  staffNames[<?php echo $st['id']; ?>] = '<?php echo addslashes($st['name'].' '.$st['surname']); ?>';
  <?php endforeach; ?>
  ...
  $('#btn-load-slots').on('click', function(){
    ...
    for (var k=0; k<staff_ids.length; k++) {
      postData['absent_staff_ids['+k+']'] = staff_ids[k];
    }
    $.post('<?php echo site_url('admin/tt/get_absent_slots'); ?>', postData,
      function(res){
        ...
        if (res.status !== '1' || !res.slots || res.slots.length === 0) {
          $('#slots-container').html(
            ...
            + '<i class="fa fa-info-circle"></i> No periods scheduled for the selected teacher(s) on <strong>'+date+'</strong>'
            + (res.day ? ' ('+res.day+')' : '')+'.</div>');
          return;
        }
        ...
        $('#slots-summary').html('<i class="fa fa-clock-o text-warning"></i> <strong>'+pendingCount+'</strong> pending of <strong>'+res.slots.length+'</strong> slots');
        $('#btn-auto-assign-all').show();

        // Group slots by absent teacher
        var grouped = {};
        $.each(res.slots, function(i, slot) {
          var sid = slot.absent_staff_id || slot.staff_id;
          if (!grouped[sid]) grouped[sid] = [];
          grouped[sid].push(slot);
        });

        var html = '<div class="sub-day-header"><i class="fa fa-calendar-check-o"></i> &nbsp;'
          + date + ' &mdash; ' + res.day + ' &nbsp; <span style="font-weight:400;font-size:12px;">('+res.slots.length+' slot'+(res.slots.length>1?'s':'')+' across '+Object.keys(grouped).length+' teacher'+(Object.keys(grouped).length>1?'s':'')+')</span></div>';

        $.each(grouped, function(absent_id, slots) {
          html += '<div style="margin:14px 0 6px 0;padding:8px 14px;background:#eef5fb;border-radius:4px;font-size:13px;font-weight:600;color:#2c3e50;">'
            + '<i class="fa fa-user-times text-danger"></i> '+(staffNames[absent_id]||'Teacher #'+absent_id)
            + ' <span style="font-weight:400;color:#888;margin-left:6px;">('+slots.length+' period'+(slots.length>1?'s':'')+')</span></div>'
          ...
          $.each(slots, function(i, slot){
            ...
            html += '<div class="col-md-6 col-lg-4">'
              + '<div class="slot-card '+(isAssigned?'assigned':'pending')+'" data-entry-id="'+slot.id+'" data-period-id="'+slot.period_id+'" data-absent-id="'+absent_id+'" data-sub-id="'+(slot.substitution?slot.substitution.id:0)+'">'
  ...
  $(document).on('click', '.btn-assign-sub', function(){
    ...
    var absent_id = $card.data('absent-id') || $('#absent_staff').val();
    if (Array.isArray(absent_id)) absent_id = absent_id[0];
    var sub_id   = $card.find('.slot-sub-select').val();
    var note     = $card.find('.slot-note').val();
    var sub_row_id = $card.data('sub-id') || 0;
    _saveSubstitution(entry_id, absent_id, sub_id, note, sub_row_id, $card);
  });
  ...
  // Bulk auto-assign all (priority solver)
  $('#btn-auto-assign-all').on('click', function(){
    var staff_ids = $('#absent_staff').val();
    var date = $('#absence_date').val();
    if (!staff_ids || staff_ids.length === 0) return;

    var $btn = $(this).prop('disabled',true).html('<i class="fa fa-spinner fa-spin"></i> Solving...');
    var postData = {date: date};
    postData[csrf_name] = csrf_val;
    for (var k=0; k<staff_ids.length; k++) {
      postData['absent_staff_ids['+k+']'] = staff_ids[k];
  ...
  function _saveSubstitution(entry_id, absent_id, sub_id, note, existing_id, $card) {
