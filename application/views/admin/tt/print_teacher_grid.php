...
<head>
<meta charset="UTF-8">
<title><?php echo htmlspecialchars(($staff->name ?? '').' '.($staff->surname ?? '')); ?> &mdash; Teacher Timetable</title>
<style>
...
h2 { text-align: center; font-size: 15px; margin: 8px 0 2px; color: #2c3e50; }
h3 { text-align: center; font-size: 12px; color: #666; font-weight: normal; margin-bottom: 10px; }
...
.cls-name { font-size: 10px; font-weight: bold; display: block; margin-top: 2px; }
...
<div class="header-wrap" style="text-align:center;">
  <h2 style="font-size:17px;margin:4px 0;"><?php echo htmlspecialchars($school_name); ?></h2>
</div>
<?php endif; ?>
  ...
  <thead>
    <tr>
      ...
      <th><?php echo htmlspecialchars($day_name); ?>
        <?php if (!empty($day_dates[$day_name])): ?><br><small style="font-weight:normal;font-size:9px;"><?php echo $day_dates[$day_name]; ?></small><?php endif; ?>
      </th>
      <?php endforeach; ?>
    </tr>
  </thead>
