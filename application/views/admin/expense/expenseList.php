    ...
    <section class="content">
        ...
        <div class="fc-results">
            <div class="fc-results-header">
                <h4><i class="fa fa-credit-card"></i> <?php echo $this->lang->line('expense_list'); ?></h4>
                <?php if ($this->rbac->hasPrivilege('expense', 'can_add')) { ?>
                    <button type="button" class="btn-add-new" onclick="openAddModal()">
                        <i class="fa fa-plus"></i> <?php echo $this->lang->line('add_expense'); ?>
                    </button>
                <?php } ?>
            </div>
...
<div id="crudModal" class="crud-modal-overlay" onclick="if(event.target===this)closeModal();">
    <div class="crud-modal">
        <div class="crud-modal-header">
            <h3><?php echo $this->lang->line('add_expense'); ?></h3>
            <button type="button" class="crud-modal-close" onclick="closeModal();">&times;</button>
        </div>
        <form id="form1" action="<?php echo base_url(); ?>admin/expense" method="post" accept-charset="utf-8" enctype="multipart/form-data">
            <div class="crud-modal-body">
                ...
                <div class="form-grp">
                    ...
                    <select id="exp_head_id" name="exp_head_id" class="crud-select" required>
                        <option value=""><?php echo $this->lang->line('select'); ?></option>
                        <?php foreach ($expheadlist as $exphead) { ?>
                            <option value="<?php echo $exphead['id']; ?>" <?php if (set_value('exp_head_id') == $exphead['id']) echo 'selected="selected"'; ?>><?php echo $exphead['exp_category']; ?></option>
                        <?php } ?>
                    </select>
                ...
                <div class="form-grp">
                    ...
                    <input type="text" id="date" name="date" class="crud-input date" value="<?php echo set_value('date', date($this->customlib->getSchoolDateFormat())); ?>" readonly="readonly" required />
...
$(function() {
    ...
    <?php if (form_error('exp_head_id') || form_error('name') || form_error('date') || form_error('amount') || isset($error_message)) { ?>
        openAddModal();
    <?php } ?>
});
