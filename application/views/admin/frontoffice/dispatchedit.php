            ...
            <div class="col-md-12">
                ...
                <div class="box box-primary">
                    <div class="box-header ptbnull">
                        <h3 class="box-title titlefix"><?php echo $this->lang->line('postal_dispatch_list'); ?> <?php echo $this->lang->line('list'); ?></h3>
                        <div class="box-tools pull-right">
                        </div><!-- /.box-tools -->
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <div class="download_label"><?php echo $this->lang->line('postal_dispatch_list'); ?> <?php echo $this->lang->line('list'); ?></div>
...
<!-- Edit Postal Dispatch Modal -->
<?php if ($this->rbac->hasPrivilege('postal_dispatch', 'can_add') || $this->rbac->hasPrivilege('postal_dispatch', 'can_edit')) { ?>
<div id="editDispatchModal" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" onclick="window.location.href='<?php echo site_url('admin/dispatch'); ?>'">&times;</button>
                <h4 class="modal-title"><?php echo $this->lang->line('edit_postal_dispatch'); ?></h4>
            </div>
            <form id="form1" action="<?php echo site_url('admin/dispatch/editdispatch/' . $Dispatch_data['id']) ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?php echo $Dispatch_data['id']; ?>">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" value="<?php echo set_value('from', $Dispatch_data['from_title']); ?>"  name="from">
                        <span class="text-danger"><?php echo form_error('from'); ?></span>
                    </div>
                    <div class="form-group">
                        <label for="pwd"><?php echo $this->lang->line('date'); ?></label>
                        <input id="date" name="date" placeholder="" type="text" class="form-control date"  value="<?php if($Dispatch_data['date']){ echo set_value('date', date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($Dispatch_data['date']))); } ?>" readonly="readonly" />
                ...
                <div class="modal-footer">
                    <a href="<?php echo site_url('admin/dispatch'); ?>" class="btn btn-default"><?php echo $this->lang->line('cancel'); ?></a>
...
<script>
    $(document).ready(function () {
        $('#editDispatchModal').modal({backdrop: 'static', keyboard: false, show: true});
    });
</script>
