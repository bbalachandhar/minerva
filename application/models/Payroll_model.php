...
class Payroll_model extends MY_Model
{
    ...
    * @param bool $only_paid Include only paid payslips in YTD (default true)
    ...
    private function getOnDutyLeaveTypeIds()
    {
        ...
        if ($has_credit_source_col) {
            $this->db->select('credit_source_type_id');
        }
        ...
        foreach ($rows as $row) {
            if ($has_balance_flag) {
                $is_credit_earner = ((int) ($row['requires_balance_check'] ?? 1) === 0);
                $is_credit_consumer = $has_credit_source_col && !empty($row['credit_source_type_id']);

                // OD set must include only credit-earning types, not consumer types like CPL.
                if ($is_credit_earner && !$is_credit_consumer) {
    ...
    private function buildLopAdjustmentLeavePool($staff_id, $od_type_ids, $od_balances)
    {
        ...
        // Rule: LOP adjustment eligibility is based only on leave_types.is_lop = 0.
        // Do not exclude by requires_balance_check / pre-allotted settings here.

        // Keep credit-consumer types (e.g. CPL) in the LOP adjustment pool.
        // Business flow: CPL should be eligible to offset LOP if balance is available.
    ...
    public function getOrCreateMonthlyBalance($staff_id, $leave_type_id, $year, $month)
    {
        ...
        if ($prev_query->num_rows() > 0) {
            ...
            // closing_balance already incorporates admin_adjustment (new design).
            // Just use it directly — do NOT add admin_adjustment again.
            $opening_balance = (float) $prev_balance['closing_balance'];
        } else {
            ...
            if (!empty($latest_prev)) {
                $opening_balance = (float) $latest_prev['closing_balance'];
        ...
        // Create new record. admin_adjustment starts at 0 — admin can set it via
        // Update Leave Balance page before payroll runs.
        $data = [
            'staff_id' => $staff_id,
            'leave_type_id' => $leave_type_id,
            'year' => $year,
