...
class Classsection_model extends MY_Model
{
    ...
    public function getClassSectionStudentCount($department_id = null)
    {
        $class_section_array = $this->customlib->get_myClassSectionQuerystring('class_sections');
		$userdata = $this->customlib->getUserData();
        $query = "SELECT class_sections.*,classes.class,sections.section,(SELECT COUNT(*) FROM student_session INNER JOIN students on students.id=student_session.student_id WHERE student_session.class_id=classes.id and student_session.section_id=sections.id and students.is_active='yes'  and student_session.session_id=" . $this->current_session . " )  as student_count,(SELECT COUNT(*) FROM student_session INNER JOIN students on students.id=student_session.student_id WHERE student_session.class_id=classes.id and student_session.section_id=sections.id and students.is_active='yes' and students.gender='Male' and student_session.session_id=" . $this->current_session . " ) as male_count,(SELECT COUNT(*) FROM student_session INNER JOIN students on students.id=student_session.student_id WHERE student_session.class_id=classes.id and student_session.section_id=sections.id and students.is_active='yes' and students.gender='Female' and student_session.session_id=" . $this->current_session . " ) as female_count FROM (SELECT MIN(id) as id, class_id, section_id FROM `class_sections` GROUP BY class_id, section_id) as class_sections INNER JOIN classes on classes.id=class_sections.class_id INNER JOIN sections on sections.id=class_sections.section_id  where 0=0 " . $class_section_array;
        if ($department_id != null) {
            $query .= " AND classes.department_id = " . $this->db->escape($department_id);
        }
        $query .= " ORDER by classes.class ASC, sections.section asc";
