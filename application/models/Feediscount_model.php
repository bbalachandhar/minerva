...
class Feediscount_model extends MY_Model
{
    ...
    public function getStudentFeesDiscountBatch(array $student_session_ids)
    {
        if (empty($student_session_ids)) return [];
        $this->db->select('student_fees_discounts.id, student_fees_discounts.student_session_id, student_fees_discounts.status, student_fees_discounts.payment_id, student_fees_discounts.description as student_fees_discount_description, student_fees_discounts.fees_discount_id, student_fees_discounts.custom_amount, fees_discounts.name, fees_discounts.code, fees_discounts.amount, fees_discounts.description, fees_discounts.session_id, fees_discounts.type, fees_discounts.percentage');
        $this->db->from('student_fees_discounts');
        $this->db->join('fees_discounts', 'fees_discounts.id = student_fees_discounts.fees_discount_id');
        $this->db->where_in('student_fees_discounts.student_session_id', $student_session_ids);
        $this->db->order_by('student_fees_discounts.student_session_id');
        $this->db->order_by('student_fees_discounts.id');
    ...
    {
        $query = "SELECT * FROM (SELECT student_fees_discounts.*, fees_discounts.name, fees_discounts.code, fees_discounts.type, fees_discounts.percentage, fees_discounts.amount, fees_discounts.discount_limit, IFNULL(applied_fees.total_assigned, 0) AS total_assigned, IF(EXISTS(SELECT 1 FROM student_fees_deposite sfd LEFT JOIN student_fees_master sfm ON sfm.id = sfd.student_fees_master_id WHERE sfm.student_session_id = student_fees_discounts.student_session_id AND sfd.amount_detail LIKE CONCAT('%', COALESCE(NULLIF(fees_discounts.code,''), fees_discounts.name), '%')), 1, 0) AS total_assigned_from_deposit, fees_discounts.discount_limit - (IFNULL(applied_fees.total_assigned, 0) + IF(EXISTS(SELECT 1 FROM student_fees_deposite sfd LEFT JOIN student_fees_master sfm ON sfm.id = sfd.student_fees_master_id WHERE sfm.student_session_id = student_fees_discounts.student_session_id AND sfd.amount_detail LIKE CONCAT('%', COALESCE(NULLIF(fees_discounts.code,''), fees_discounts.name), '%')), 1, 0)) AS remaining_discount_limit, fees_discounts.expire_date FROM `student_fees_discounts` INNER JOIN fees_discounts ON fees_discounts.id = student_fees_discounts.fees_discount_id LEFT JOIN (SELECT COUNT(*) AS total_assigned, student_fees_discount_id FROM `student_applied_discounts` GROUP BY student_fees_discount_id) AS applied_fees ON applied_fees.student_fees_discount_id = student_fees_discounts.id WHERE student_fees_discounts.student_session_id = " . $this->db->escape($student_session_id) . " and (fees_discounts.expire_date >=CURDATE() or fees_discounts.expire_date is NULL) ) AS subquery WHERE remaining_discount_limit > 0;";
