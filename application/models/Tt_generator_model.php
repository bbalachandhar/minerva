...
class Tt_generator_model extends MY_Model
{
    ...
    /**
     * Gap-fill pass: after the greedy algorithm has done its best, scan every
     * remaining empty cell for each class in scope.
     *
     * Step A – try to place an extra occurrence of a configured subject whose
     *          teacher is available right now (respects weekly/daily caps, unavailability,
     *          subject time-off and max_per_day; single-period only).
     * Step B – if nothing fits, stamp a generic "Free Period" placeholder so the
     *          printed timetable has no blank cells.
     *
     * Updates class_occ / teacher_occ / teacher_periods_* in place; appends to
     * $draft_entries; increments $gap_filled_subject / $gap_filled_free counters.
     */
    private function _fillEmptyCells(
        array  $class_stats,
        array  $base_loads,
        array  $constraints,
        array  $unavail_map,
        ...
        int    $session_id,
        array &$draft_entries,
        int   &$gap_filled_subject,
        int   &$gap_filled_free
    ): void {
        // Count already-placed periods per (class, section, sgs_id) in this pass
        $placed_count = [];
        foreach ($draft_entries as $de) {
            if (!empty($de['subject_group_subject_id'])) {
                $pk = $de['class_id'] . '_' . $de['section_id'] . '_' . $de['subject_group_subject_id'];
                $placed_count[$pk] = ($placed_count[$pk] ?? 0) + 1;
            }
        }

        // Index loads by class+section key
        $loads_by_class = [];
        foreach ($base_loads as $load) {
            $ck = (int)$load->class_id . '_' . (int)$load->section_id;
            $loads_by_class[$ck][] = $load;
        }

        foreach ($class_stats as $cs_stat) {
            $class_id   = (int) $cs_stat['class_id'];
            $section_id = (int) $cs_stat['section_id'];
            $ck         = $class_id . '_' . $section_id;

            $class_loads = $loads_by_class[$ck] ?? [];

            // Sort once per class: most-needed subjects first
            usort($class_loads, function($a, $b) use ($placed_count, $ck) {
                $pk_a = $ck . '_' . (int)$a->subject_group_subject_id;
                $pk_b = $ck . '_' . (int)$b->subject_group_subject_id;
                $need_a = max(0, (int)$a->periods_per_week - ($placed_count[$pk_a] ?? 0));
                $need_b = max(0, (int)$b->periods_per_week - ($placed_count[$pk_b] ?? 0));
                return $need_b <=> $need_a;
            });

            foreach ($this->working_days as $day) {
                foreach ($this->period_order as $pid) {
                    // Check if slot is empty across ALL batch keys
                    $slot_occupied = false;
                    if (!empty($this->class_occ[$class_id][$section_id][$day][$pid])) {
                        foreach ($this->class_occ[$class_id][$section_id][$day][$pid] as $val) {
                            if ($val) { $slot_occupied = true; break; }
                        }
                    }
                    if ($slot_occupied) continue;
                    if (!empty($this->class_unavail[$class_id][$section_id][$day][$pid])) continue;

                    // --- Step A: try to place a subject ---
                    $filled = false;
                    foreach ($class_loads as $load) {
                        // Only single-period fills — consecutive blocks need contiguous free cells
                        if ((int)$load->consecutive_periods > 1) continue;

                        $sgs_id = (int) $load->subject_group_subject_id;
                        $pk     = $ck . '_' . $sgs_id;

                        if (($placed_count[$pk] ?? 0) >= (int)$load->periods_per_week) continue;

                        $day_subj_count = $this->subject_day_count[$class_id][$section_id][$sgs_id][$day] ?? 0;
                        if ($day_subj_count >= max(1, (int)($load->max_per_day ?? 2))) continue;

                        if (!empty($this->subject_unavail[$sgs_id][$day][$pid])) continue;

                        // Try each teacher in the pool
                        $teacher_ids = $load->teacher_ids ?? [];
                        if (empty($teacher_ids) && !empty($load->staff_id)) {
                            $teacher_ids = [(int)$load->staff_id];
                        }

                        foreach ($teacher_ids as $t_id) {
                            if (empty($t_id)) continue;
                            $c = $constraints[$t_id] ?? $this->default_tc;

                            $week_used = $this->teacher_periods_week[$t_id] ?? 0;
                            if ($c->max_periods_per_week > 0 && $week_used >= (int)$c->max_periods_per_week) continue;

                            $day_used = $this->teacher_periods_day[$t_id][$day] ?? 0;
                            $eff_cap  = min(
                                (int)$c->max_periods_per_day,
                                count($this->period_order) - max(0, (int)($c->min_free_per_day ?? 0))
                            );
                            if ($day_used + 1 > $eff_cap) continue;

                            if (!empty($this->teacher_occ[$t_id][$day][$pid])) continue;
                            if (!empty($unavail_map[$t_id][$day][$pid]))        continue;

                            // Place it
                            $batch_id  = !empty($load->batch_id) ? (int)$load->batch_id : null;
                            $batch_key = $batch_id ?: 0;

                            $this->class_occ[$class_id][$section_id][$day][$pid][$batch_key] = true;
                            $this->teacher_occ[$t_id][$day][$pid]                             = true;
                            $this->teacher_periods_day[$t_id][$day]                           = $day_used + 1;
                            $this->teacher_periods_week[$t_id]                                = $week_used + 1;
                            $this->subject_day_count[$class_id][$section_id][$sgs_id][$day]  = $day_subj_count + 1;
                            ...
                            $gap_filled_subject++;
                            $filled = true;
                            break 2; // move to next day/period cell
                        }
                    }

                    // --- Step B: no subject fits → Free Period placeholder ---
                    if (!$filled) {
                        $this->class_occ[$class_id][$section_id][$day][$pid][0] = true;
                        $draft_entries[] = [
                            'gen_log_id'               => $log_id,
