...
class Cbseexam_template_model extends MY_Model
{
    ...
    */
    ...
    public function all_term_old($template_id)
    {
         $userdata = $this->customlib->getUserData();
            $role_id = $userdata["role_id"];
            $carray = array();
    ...
    public function all_term($template_id)
    {
        ...
        $this->db->select('cbse_terms.*')
        ->from('cbse_exam_class_sections')
        ->join('cbse_template_class_sections', 'cbse_template_class_sections.class_section_id=cbse_exam_class_sections.class_section_id')
        ->join('cbse_exams', 'cbse_exams.id=cbse_exam_class_sections.cbse_exam_id')
        ->join('cbse_terms', 'cbse_terms.id=cbse_exams.cbse_term_id')
        ->where('cbse_exams.session_id', $this->current_session)
        ->where('cbse_template_class_sections.cbse_template_id', $template_id)
        ->where('cbse_exams.session_id', $this->current_session);
        ...
        foreach ($result as $key => $value) {
            ...
            ->from('cbse_exams')
            ->join('cbse_exam_class_sections', 'cbse_exams.id=cbse_exam_class_sections.cbse_exam_id')
            // ->join('cbse_template_class_sections', "cbse_template_class_sections.class_section_id=cbse_exam_class_sections.class_section_id and cbse_template_class_sections.cbse_template_id=$template_id")
            ->where('cbse_term_id',   $value['id'])
            ->where('cbse_exams.session_id', $this->current_session);
    ...
    public function getTemplateTermExamWithAssessment($cbse_template_id)
    {
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $result_key => $result_value) {
                if(isset($result_value->subject_code) && $result_value->subject_code!=""){
                    $subjects[$result_value->subject_id] = $result_value->subject_name . " (" . $result_value->subject_code . ")";
                }else{
                    $subjects[$result_value->subject_id] = $result_value->subject_name ;
                }
            }
    ...
    */
    ...
    //==============================
    public function get_single_exam_template_by_sectionid($class_section_id)
    {
            return $this->db->select('cbse_template.*')->from('cbse_template')
            ->join('cbse_template_class_sections', 'cbse_template.id=`cbse_template_class_sections`.`cbse_template_id`')
            // ->where('marksheet_type is NOT NULL')->where('gradeexam_id is NOT NULL')->where('remarkexam_id is NOT NULL')
            ->where("`cbse_template_class_sections`.`class_section_id` in ( $class_section_id)")
            ->where('`cbse_template`.`session_id`', $this->current_session)
            ->where('`cbse_template`.`marksheet_type`', 'exam_wise')
            ->group_by("cbse_template.id")
            ->get()
            ->result_array();
    }
    //==============================
