...
class Cbse_mail_sms
{
    ...
    {
        if ($chk_mail_sms['mail'] || $chk_mail_sms['sms'] || $chk_mail_sms['notification'] || $chk_mail_sms['whatsapp']) {
            ...
            if (!empty($exam_result['exam_result'])) {
                foreach ($exam_result['exam_result'] as $res_key => $res_value) {
                    ...
                    $token = $this->encode_marksheet_token($res_value['student_session_id'], $exam_result['marksheet_template'], $exam_result['exam']['id']);
                    $marksheet_pdf_url = base_url() . "download-marksheet/" . $token;
                    //==================
                    ...
                    $detail = array(
                        ...
                        'exam_marksheet_url' => $marksheet_pdf_url
                    );
                    ...
                    //whatsapp msg
                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && $chk_mail_sms['student_recipient']) {
                        $this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sentCBSEExamResultWhatsapp($detail, $template, $chk_mail_sms, $detail['mobileno'], $chk_mail_sms['whatsapp_template_id']);
                    }

                    if ($chk_mail_sms['whatsapp'] && $chk_mail_sms['template'] != "" && $chk_mail_sms['guardian_recipient']) {
                        $this->CI->load->library('whatsappgateway');
                        $this->CI->whatsappgateway->sentCBSEExamResultWhatsapp($detail, $template, $chk_mail_sms, $detail['guardian_phone'], $chk_mail_sms['whatsapp_template_id']);
                    }
                    //whatsapp msg
    ...
    //======================
    private function encode_marksheet_token($student_session_id, $marksheet_template, $exam_id)
    {
        $data = implode('|', [
            $student_session_id,
            $marksheet_template,
