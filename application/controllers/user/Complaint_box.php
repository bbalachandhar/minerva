...
class Complaint_box extends Student_Controller
{
    ...
    /**
     * List own complaints + submit form.
     */
    public function index()
    {
        if (!$this->studentmodule_lib->hasActive('complaint_box')) {
        ...
        $student_session_id   = $this->session->userdata['current_class']['student_session_id'];
        $data['complaints']   = $this->complaint_box_model->getByStudentSession($student_session_id);
        $data['complaint_types'] = $this->complaint_Model->getComplaintType();
        $this->load->view('layout/student/header', $data);
        $this->load->view('user/complaint_box/index', $data);
        $this->load->view('layout/student/footer', $data);
    ...
    public function add()
    {
        ...
        $this->form_validation->set_rules('complaint_type_id',  'Complaint Type',   'trim|required|integer');
        $this->form_validation->set_rules('description',        'Description',      'trim|required|xss_clean');
        $this->form_validation->set_rules('priority',           'Priority',         'trim|required|in_list[low,medium,high]');
        ...
        if ($this->form_validation->run() == false) {
            echo json_encode([
                ...
                'error'   => [
                    ...
                    'complaint_type_id' => form_error('complaint_type_id'),
                    'description'       => form_error('description'),
                    'priority'          => form_error('priority'),
                ],
                'message' => '',
        ...
        $attachment = '';
        if (isset($_FILES['attachment']) && $_FILES['attachment']['name'] != '' && $_FILES['attachment']['error'] == 0) {
            $this->customlib->ensureDirectoryExists('./uploads/complaint_box/');
            $upload_result = $this->media_storage->fileupload('attachment', './uploads/complaint_box/');
            if ($upload_result['status'] === false) {
                echo json_encode(['status' => 'fail', 'error' => ['attachment' => $upload_result['message']], 'message' => '']);
                return;
            }
            $attachment = $upload_result['message'];
        }

        $data = [
            'student_session_id' => $student_session_id,
            'submitted_by'       => ($role === 'parent') ? 'parent' : 'student',
            'complaint_type_id'  => (int)$this->input->post('complaint_type_id'),
        ...
        $id = $this->complaint_box_model->add($data);
    ...
    /**
     * AJAX: get detail of a single complaint (owned by this student).
     */
    public function get_detail($id)
    {
        $id                 = (int)$id;
        $student_session_id = $this->session->userdata['current_class']['student_session_id'];
        $row                = $this->complaint_box_model->getById($id);
