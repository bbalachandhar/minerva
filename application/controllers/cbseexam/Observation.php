...
class Observation extends MY_Addon_CBSEController
{
        ...
        if (!isset($row)) {
            ...
            foreach ($row as $row_key => $row_value) {
                foreach ($cbse_observation_parameters as $parameter_key => $parameter_value) {
                    ...
                    if ($this->input->post('param_value_' . $student_session . '_' . $parameter_value) == "") {
    ...
    public function termstudent()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $studentlist = $this->cbseexam_exam_model->searchStudentOverservationMarks($cbse_observation_term_id, $data['class_id'], $data['section_id']);
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('cbse_exam_assign_observation', 'can_edit')) {
                    ...
                    $studentsbtn = "<button type='button' data-record_id='" . $value->id . "' data-cbse_term_id='" . $value->cbse_term_id . "'  class='btn btn-primary btn-xs' data-action='insert' data-toggle='modal' data-target='#assignModal' data-original-title='".$this->lang->line('assign_marks')."' title='".$this->lang->line('assign_marks')."' autocomplete='off'><i class='fa fa-newspaper-o'></i></button> ";

                    $editbtn = "<button type='button' data-record_id='" . $value->id . "' class='btn btn-primary btn-xs' data-action='update' data-toggle='modal' data-target='#myModal' data-original-title='".$this->lang->line('edit_observation_term')."' title='".$this->lang->line('edit_observation_term')."' autocomplete='off'><i class='fa fa-pencil'></i></button> ";

                }   

                if ($this->rbac->hasPrivilege('cbse_exam_assign_observation', 'can_delete')) {
                    $deletebtn = '';
                    $deletebtn = "<a onclick='return confirm("."\"". $this->lang->line('delete_confirm') ."\"". ")' href='" . base_url() . "cbseexam/observation/deleteassignterm/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                }
