...
class Exam extends MY_Addon_CBSEController
{
 ...
 public function getByClasssection()
    ...
    public function add()
    {
        ...
        $this->form_validation->set_rules('cbse_category_id', $this->lang->line('category'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $msg['cbse_category_id'] = form_error('cbse_category_id');
            ...
            if ($this->input->post('use_exam_roll_no')!=null) {
                $use_exam_roll_no = 1;
            } else {
                $use_exam_roll_no = 0;
            }
            ...
            $data = array(
                ...
                'cbse_category_id' => $this->input->post('cbse_category_id'),
                'is_active' => $is_active,
                'is_publish' => $is_publish,
                'created_by' => $this->customlib->getStaffID(),
                'session_id' => $this->current_session,
                'use_exam_roll_no' => $use_exam_roll_no,
            );
	...
	public function exam_attendance_search()
    {	 
        $data['exam_id'] = $exam_id =  $this->input->post('exam_id');
        $data1['start'] = $this->customlib->dateFormatToYYYYMMDD($this->input->post('from_date'));
        $data1['end'] = $this->customlib->dateFormatToYYYYMMDD($this->input->post('to_date'));
        $data['exam'] = $this->cbseexam_exam_model->get_exambyId($exam_id);
        $resultlist = $this->cbseexam_exam_model->get_examstudents($exam_id);
	 
        foreach ($resultlist as $key => $student) {

            $student_total_attendances = $this->attendencetype_model->getStudentAttendenceRange($data1, $student['student_session_id']);
            ...
            if (!empty($student_total_attendances)) {
                ...
                $this->attendencetype_model->getStudentAttendenceRange($data1, $resultlist[0]['student_session_id']);
            }

            if (!empty($total_presents)) {
                $resultlist[$key]['total_present_days'] =  $total_presents;
            } else {
                $resultlist[$key]['total_present_days'] = '';
            }
        }

        $resultlist[0]['total_working_days'] = count($this->cbseexam_exam_model->getStudentAttendenceRange($data1));
    ...
    public function addattendance()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $array = array('status' => '1', 'error' => array(), 'message' => $this->lang->line('success_message'));
        }
    ...
    public function get_exam()
    {
        ...
        //=========================added============================//
        $class_section_name = $this->cbseexam_exam_model->get_cbse_exam_sections_name($exam_id);
        $sections=$class_section_name[0]['sections'];
        $data['template_list']  = $this->cbseexam_template_model->get_single_exam_template_by_sectionid($sections);
        //=========================added============================//
    ...
    public function edit()
    {
        ...
        //=========================added============================//
        // if (isset($_POST['is_publish'])){ 
            // $this->form_validation->set_rules('template', $this->lang->line('template'), 'trim|required|xss_clean');//added
        // }
        //=========================added============================//
        ...
        if ($this->form_validation->run() == false) {
            ...
            //================
            // if (isset($_POST['is_publish']) && $this->input->post('template')!=""){ 
			if (isset($_POST['is_publish'])){ 
                $marksheet_template=$this->input->post('template');
                $exam = $this->cbseexam_exam_model->get_exambyId($this->input->post('exam_id'));
				
				 
                $exam_students = $this->cbseexam_exam_model->get_examstudents($this->input->post('exam_id'));
                $student_exams = array('exam' => $exam, 'exam_result' => $exam_students,'marksheet_template'=>$marksheet_template);
			...
			if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
				...
				if ($_FILES["file"]["size"] > 0) {
					...
					while (($column = fgetcsv($file, 10000, ",")) !== false) {
						if ($flag) {
							// Save the header row for dynamic marks keys
							$header = $column;
							$flag = false;
							continue;
						}
	
						// Skip empty adm_no or status columns
						...
						$row_data = [
							'adm_no' => $column[0],
							'attendence' => $column[1],
							'note' => isset($column[array_search('note', $header)]) ? $this->encoding_lib->toUTF8($column[array_search('note', $header)]) : '',
						];
	
						// Collect all marks columns dynamically
						foreach ($header as $key => $head_col) {
							if (preg_match('/^marks_\d+$/', $head_col)) {
								$row_data['marks'][] = isset($column[$key]) ? $column[$key] : '';
							}
						}
	
						$return_array[] = json_encode($row_data);
					}
					fclose($file);
				}
	...
	public function exportformat()
    {
        ...
        $filepath = "./backend/import/import_cbse_marks_sample_file.csv";
