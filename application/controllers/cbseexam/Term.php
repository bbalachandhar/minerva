...
class Term extends MY_Addon_CBSEController
{
    ...
    public function index()
    {
		...
		$this->session->set_userdata('subsub_menu', 'cbseexam/term');
        ...
        if (!empty($m->data)) {
            foreach ($m->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('cbse_exam_term', 'can_edit')) {
                    $editbtn = "<button  class='btn btn-primary btn-xs edit_term'  data-toggle='tooltip' data-recordid=" . $value->id . "  title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></button>";
                }

                if ($this->rbac->hasPrivilege('cbse_exam_term', 'can_delete')) {
                    $deletebtn = "<a onclick='return confirm(" . "\"" . $this->lang->line('delete_confirm') . "\"" . ")' href='" . base_url() . "cbseexam/term/delete/" . $value->id . "' class='btn btn-primary btn-xs' title='" . $this->lang->line('delete') . "' data-toggle='tooltip'><i class='fa fa-trash'></i></a>";
                }
