...
//==========================
    ...
    public function activate_biometric_device($id) {
        ...
        public function sync_biometric_attendance() {
            log_message('debug', 'Staff::sync_biometric_attendance - Sync process started.');

            if (!($this->rbac->hasPrivilege('biometric_device', 'can_view'))) {
                log_message('error', 'Staff::sync_biometric_attendance - Permission denied.');
                access_denied();
            }
            ...
            if (!$active_device) {
                ...
                log_message('error', 'Staff::sync_biometric_attendance - No active biometric device found.');
                redirect('admin/staffattendance/index');
            }
            log_message('debug', 'Staff::sync_biometric_attendance - Active device found: ' . json_encode($active_device));

            $last_sync_datetime = $this->attendance_model->get_last_biometric_sync_datetime();
            log_message('debug', 'Staff::sync_biometric_attendance - Last sync datetime from model: ' . ($last_sync_datetime ? $last_sync_datetime : 'NULL'));

            $from_datetime = $last_sync_datetime ? $last_sync_datetime : date('Y-m-01 00:00:00');
            $to_datetime = date('Y-m-d H:i:s');
    
            log_message('debug', 'Staff::sync_biometric_attendance - Fetching punches with params: From: ' . $from_datetime . ', To: ' . $to_datetime);

            $punches = $this->biometric_api_model->get_punches_from_api($active_device, $from_datetime, $to_datetime);
    
            log_message('debug', 'Staff::sync_biometric_attendance - Punches received from API model: ' . json_encode($punches));

            if ($punches !== false) {
                $inserted_count = $this->attendance_model->save_raw_biometric_punches($punches);
                log_message('debug', 'Staff::sync_biometric_attendance - Inserted count from model: ' . $inserted_count);
    
                $this->attendance_model->update_last_biometric_sync_datetime($to_datetime);
                log_message('info', 'Staff::sync_biometric_attendance - Updated last sync datetime to: ' . $to_datetime);
                ...
                log_message('info', 'Staff::sync_biometric_attendance - Sync successful. Redirecting.');
                redirect('admin/staffattendance/index');
            } else {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger">' . $this->lang->line('biometric_sync_failed') . '</div>');
                log_message('error', 'Staff::sync_biometric_attendance - API model returned false. Sync failed.');
                redirect('admin/staffattendance/index');
            }
        }
