...
class Onlineadmission extends Admin_Controller
{
    ...
    public function course_import_template()
    {
        ...
        fputcsv($out, ['ME - Applied Electronics','ME-AE','PG','first_year','','','2','','2']);
    ...
    public function bulk_import_courses()
    {
        ...
        if (!$handle) { echo json_encode(['status' => 0, 'message' => 'Could not read file']); return; }
        fgetcsv($handle); // skip header row

        $inserted = 0; $updated = 0; $skipped = 0; $errors = []; $row_num = 1;
        while (($row = fgetcsv($handle)) !== false) {
            ...
            if (empty($name)) { $errors[] = "Row $row_num: course_name required"; $skipped++; continue; }
            if (!in_array($level, ['UG','PG'])) { $errors[] = "Row $row_num: level must be UG or PG"; $skipped++; continue; }
            if (!in_array($type, ['first_year','lateral'])) $type = 'first_year';

            $data = [
                'course_name'    => $name,   'course_code'    => $code,
                'course_level'   => $level,  'admission_type' => $type,
                'govt_fee'       => $govt_fee, 'mgt_fee'      => $mgt_fee,
            ...
            if (!empty($code)) {
                ...
                if ($existing) { $data['id'] = $existing['id']; $this->Onlineadmissioncourses_model->add($data); $updated++; continue; }
            }
